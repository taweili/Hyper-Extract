...
> **一个智能的、基于 LLM 的知识提取框架**
>
> 借助大语言模型的力量，将非结构化文本转化为结构化的知识容器。

---

## 🎯 愿景

**Hyper-Extract** 提供一个强大的智能知识提取框架。我们不是将数据结构视为被动容器，而是赋予它们 **LLM 驱动的智能**，使其能够主动地从非结构化文本中提取、合并和索引知识。

Hyper-Extract 中的每个知识容器都遵循统一的生命周期：

1. **Extract（提取）** - 使用 LLM 从文本智能提取，自动处理分块和合并
2. **Merge（合并）** - 通过可配置的合并策略实现自动聚合
...
- **生命周期管理**：标准化的 `extract` → `merge` → `search` → `dump/load` 流程

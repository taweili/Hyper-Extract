"""
演示 ListKnowledge 的 Pythonic 列表操作

这个示例展示了如何像使用 Python 原生 list 一样使用 ListKnowledge：
- 索引访问和切片
- 迭代和成员测试
- 追加、插入和扩展
- 删除和弹出元素
...
def main():
    ...
    print("=" * 70)
    print("ListKnowledge Pythonic 操作演示")
    print("=" * 70)

    # 1. 使用 append 添加任务
    print("\n1. 使用 append() 添加任务:")
    tasks.append(Task(name="实现用户登录", priority=5, status="done"))
    tasks.append(Task(name="设计数据库", priority=4, status="done"))
    tasks.append(Task(name="编写API文档", priority=3, status="in_progress"))
    print(f"   已添加 {len(tasks)} 个任务")

    # 2. 索引访问
    print("\n2. 索引访问:")
    print(f"   第一个任务: {tasks[0].name} (优先级 {tasks[0].priority})")
    print(f"   最后一个任务: {tasks[-1].name} (状态: {tasks[-1].status})")

    # 3. 切片
    print("\n3. 切片操作:")
    high_priority = tasks[:2]
    print(f"   前两个高优先级任务:")
    ...
    print(f"   测试任务在列表中: {test_task in tasks}")

    # 7. 插入任务
    print("\n7. 使用 insert() 在开头插入紧急任务:")
    tasks.insert(0, Task(name="修复生产BUG", priority=5, status="in_progress"))
    print(f"   新的第一个任务: {tasks[0].name}")
    ...
    # 10. 修改任务
    print("\n10. 修改任务状态 (通过索引赋值):")
    print(f"   修改前: {tasks[-1].name} - {tasks[-1].status}")
    updated_task = tasks[-1]
    updated_task.status = "done"
    tasks[-1] = updated_task
    print(f"   修改后: {tasks[-1].name} - {tasks[-1].status}")

    # 11. 删除任务
    print("\n11. 删除已完成的任务:")
    print(f"   删除前共有 {len(tasks)} 个任务")
    # 删除第一个任务
    del tasks[0]
    print(f"   删除后共有 {len(tasks)} 个任务")

    # 12. Pop 操作
    print("\n12. 使用 pop() 移除并返回任务:")
    last_task = tasks.pop()
    print(f"   弹出的任务: {last_task.name}")
    print(f"   剩余 {len(tasks)} 个任务")

    # 13. 复制
    print("\n13. 创建任务列表的副本:")
    tasks_backup = tasks.copy()
    tasks_backup.append(Task(name="新任务", priority=1, status="pending"))
    print(f"   原始列表: {len(tasks)} 个任务")
    ...
    # 16. 字符串表示
    print("\n16. 字符串表示:")
    print(f"   repr: {repr(tasks)}")
    print(f"   str: {str(tasks)}")

    # 17. 组合操作
    print("\n17. 组合两个 ListKnowledge (+ 操作符):")
    more_tasks = ListKnowledge(
        item_schema=Task,
        llm_client=llm,
        embedder=embedder,
    )
    ...
    combined = tasks + more_tasks
    print(f"   原始任务: {len(tasks)} 个")

"""Base merger class implementing tournament-style merge algorithm.
...
from typing import List, Any, Callable, TypeVar, Generic, Dict, Tuple, Optional
...
class BaseMerger(ABC, Generic[T]):
    ...
    Batch Optimization Example:
        - 10 unique keys, each with 4 duplicate items (2 pairs per key)
        - Without batch: 10 keys × 2 pairs = 20 separate API calls
        - With batch: Round 1 collects all 20 pairs → 1 API call
    ...
    def _cross_key_tournament_merge(self, groups: Dict[Any, List[T]]) -> List[T]:
        ...
        **Key Optimization**: Instead of merging each key independently, this method
        ...
        while any(len(items) > 1 for items in key_rounds.values()):
            round_num += 1
            all_pairs = []  # Collect pairs from ALL keys

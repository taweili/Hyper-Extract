...
*   **业绩电话会实录 (Earnings Call Transcripts)**：管理层与分析师的问答记录，包含业绩归因及前瞻性指引。

| 模板名称 | 底层原语 | 描述 | 典型应用场景 |
| :--- | :--- | :--- | :--- |
| **`QAExtraction`** | `AutoList` | **问答对提取**。将非结构化对话整理为结构化的 问题-回答人-回答摘要。 | 纪要整理、多维观点比对 |
| **`GuidanceTracker`** | `AutoModel` | **业绩指引追踪**。专门提取管理层给出的未来业绩承诺或定量展望数据。 | 预期管理、模型更新 |
| **`PerformanceAttribution`** | `AutoGraph` | **业绩归因图**。提取业绩波动背后的原因（如：原料涨价 -> 毛利受到挤压）。 | 经营分析、趋势判断 |
...
| 模板名称 | 底层原语 | 描述 | 典型应用场景 |
| :--- | :--- | :--- | :--- |
| **`SynergyMap`** | `AutoGraph` | **协同效应网络**。分析并购双方在业务板块、技术或市场上的互补关系。 | 并购价值评估、反垄断分析 |
| **`LegalRiskRegistry`** | `AutoSet` | **法律风险登记册**。去重汇总所有未决诉讼、合规漏洞及知识产权争议记录。 | 风险排查、估值打折计算 |
| **`KeyPersonnelProfile`** | `AutoModel` | **核心团队画像**。提取目标公司关键管理人员的背景、竞业条款及留任计划。 | 组织尽调、人才保留计划 |

### 3. `medicine` (医学)
专注于疾病诊疗逻辑、标准化术语映射及因果关系提取。
*   **医学教科书与专著**：系统性阐述解剖、病理、药理及治疗原则的专业书籍（如《神经科学原理》）。
...
*   **历史专著**：对特定断代、事件或人物进行深度因果分析与考证的学术书籍。
...
*   **生物学专著**：详述物种分类、进化历史或复杂生物系统机制的学术书籍。
...
### 12. `agriculture` (农业)
*   **农业技术手册**：关于作物种植规范、田间管理标准及病虫害防治技术的指导书。

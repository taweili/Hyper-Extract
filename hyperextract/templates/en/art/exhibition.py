...
class ExhibitionEntity(BaseModel):
    ...
    details: Optional[str] = Field(description="Exhibition dates, conceptual notes, or location.")
...
class ExhibitionRelation(BaseModel):
    ...
    context: Optional[str] = Field(description="Specific room, loan conditions, or thematic sub-group.")
...
EXHIBITION_NODE_PROMPT = (
    "Extract exhibition components: titles, venues, themes, specific artworks, and key personnel (curators, designers)."
)

EXHIBITION_EDGE_PROMPT = (
    "Link items within the exhibition. Show which artworks belong to which theme and identify the hosting venue and sponsors."
)

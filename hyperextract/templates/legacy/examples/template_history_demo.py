...
这个文件用于测试 hyperextract 历史领域的所有知识模板。
只需要选择模板，程序会自动匹配对应的输入文件。

使用方法：
1. 选择知识模板（修改 TEMPLATE 变量）
2. 运行文件：python examples/template_history_demo.py
"""
...
# 测试文件根目录
TEST_SAMPLES_ROOT = Path(__file__).parent.parent / "tests" / "test_data" / "test_samples" / "zh" / "history"

# 模板到输入文件的映射关系（根据 README 中的典型应用场景）
TEMPLATE_TO_INPUT = {
    # 历史专著 -> 赤壁之战
    HistoricalKnowledgeGraph: TEST_SAMPLES_ROOT / "battle_of_chibi.md",
    MultiParticipantEventMap: TEST_SAMPLES_ROOT / "battle_of_chibi.md",
    # 编年史 -> 赤壁之战
    ChronologicalEventChain: TEST_SAMPLES_ROOT / "battle_of_chibi.md",
    HistoricalContextGraph: TEST_SAMPLES_ROOT / "battle_of_chibi.md",
    PoliticalStruggleHypergraph: TEST_SAMPLES_ROOT / "battle_of_chibi.md",
    # 口述历史/信札 -> 苏轼书信
    PersonalTrajectoryHypergraph: TEST_SAMPLES_ROOT / "sushi_life_letters.md",
    NarrativeRelationGraph: TEST_SAMPLES_ROOT / "sushi_life_letters.md",
    MemoryFlashbackList: TEST_SAMPLES_ROOT / "sushi_life_letters.md",
    EpistolaryKnowledgeGraph: TEST_SAMPLES_ROOT / "sushi_life_letters.md",
}

...
class StakeholderNode(BaseModel):
    ...
    type: str = Field(
        description="类型：'自然人'、'上市公司'、'私有公司'、'国有企业'、'风险投资'。"
    )
    jurisdiction: Optional[str] = Field(
        None, description="注册地或居住地所属的国家或地区。"
    )
...
class OwnershipRelation(BaseModel):
    ...
    stake_percentage: Optional[float] = Field(
        None, description="持股比例（0.0 到 100.0）。"
    )
    control_type: str = Field(
        "Equity", description="联系性质：'股权收购'、'投票权'、'代持股份'、'直接控制'。"
    )
    as_of_date: Optional[str] = Field(
        None, description="记录该所有权的时间点或财务周期。"
    )
...
class EquityStructureGraph(AutoGraph[StakeholderNode, OwnershipRelation]):
    ...
    def show(
        ...
        *,
        ...
        """
        def node_label_extractor(node: StakeholderNode) -> str:

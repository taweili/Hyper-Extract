...
## 核心概念定义
- **节点 (Node)**：本模板中的"节点"指穴位实体或解剖标志，包括名称、所属经脉、位置描述、主治功能等信息。
- **边 (Edge)**：本模板中的"边"指穴位之间或穴位与解剖标志之间的位置关系，包括源实体、目标穴位、相对位置描述等信息。

## 提取规则
### 节点提取规则
1. 提取所有穴位实体：如合谷、足三里、三阴交等
2. 提取所有解剖标志：如脐、腕横纹、肘横纹等
3. 为每个穴位指定所属经脉
4. 提取每个穴位的位置描述和主治功能

### 关系提取规则
1. 提取穴位之间或穴位与解剖标志之间的位置关系
2. 为每种关系指定类型：位于
3. 提取相对位置描述：如脐旁2寸、腕横纹上3寸等
...
_NODE_PROMPT = """## 角色与任务
你是一位专业的针灸学专家，请从文本中提取所有穴位实体和解剖标志作为节点。
...
_EDGE_PROMPT = """## 角色与任务
你是一位专业的针灸学专家，请从给定实体列表中提取穴位的相对位置关系。

## 核心概念定义
- **节点 (Node)**：本模板中的"节点"指穴位实体或解剖标志，作为位置关系的参与者。
- **边 (Edge)**：本模板中的"边"指穴位之间或穴位与解剖标志之间的位置关系，包括源实体、目标穴位、相对位置描述等信息。
...
1. 仅从下方已知实体列表中提取边
2. 不要创建未列出的实体
3. 每条边必须包含 source、target、relationType、relativePosition

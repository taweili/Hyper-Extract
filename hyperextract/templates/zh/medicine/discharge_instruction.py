...
class DischargeInstructionInfo(BaseModel):
    """出院医嘱信息"""
    medications: List[dict] = Field(description="出院带药列表，每个药物包含名称、剂量、用法、频次等信息")
    followUp: List[dict] = Field(description="复诊计划列表，每个复诊包含时间、科室、检查项目等信息")
...
## 核心概念定义
- **对象 (Object)**：本模板中的"对象"指出院医嘱信息，包含出院带药、复诊计划、康复建议、饮食建议、活动建议和其他注意事项的结构化信息。

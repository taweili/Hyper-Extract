...
## 核心概念定义
- **节点 (Node)**：本模板中的"节点"指手术实体，包括医生、部位、术式、器械、麻醉方式等类型，用于表示手术事件中的基本要素。
- **边 (Edge)**：本模板中的"边"指手术事件超边，连接多个节点，表达完整的手术事件。

## 提取规则
### 节点提取规则
1. 提取所有手术相关实体：医生姓名、手术部位、手术术式、手术器械、麻醉方式等
2. 为每个实体指定类型：医生、部位、术式、器械、麻醉方式等
3. 为每个实体添加简要描述

### 超边提取规则
1. 仅从提取的实体中创建超边
2. 超边应表示完整的手术事件，包含医生、部位、术式、器械等多个要素
3. 为每个超边指定手术日期
4. 为每个超边添加手术时长（如果文本中提供）
5. 为每个超边添加详细描述，说明具体的手术过程
...
## 核心概念定义
- **节点 (Node)**：本模板中的"节点"指手术实体，作为手术事件的组成部分。
- **边 (Edge)**：本模板中的"边"指手术事件超边，连接多个节点，表达完整的手术事件。

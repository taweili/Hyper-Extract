...
## 核心概念定义
- **节点 (Node)**：本模板中的"节点"指相互作用实体，包括药物、体质、不良反应等类型，用于表示药物相互作用中的基本要素。
- **边 (Edge)**：本模板中的"边"指相互作用超边，连接多个节点，表达完整的药物相互作用。

## 提取规则
### 节点提取规则
1. 提取所有相互作用相关实体：药物名称、体质类型、不良反应等
2. 为每个实体指定类型：药物、体质、不良反应、其他等
3. 为每个实体添加简要描述

### 超边提取规则
1. 仅从提取的实体中创建超边
2. 超边应表示完整的药物相互作用，包含药物、体质、不良反应等多个要素
3. 为每个超边指定相互作用类型：增强、减弱、不良反应等
4. 为每个超边添加详细描述，说明具体的相互作用机制
...
## 核心概念定义
- **节点 (Node)**：本模板中的"节点"指相互作用实体，作为药物相互作用的参与者。
- **边 (Edge)**：本模板中的"边"指相互作用超边，连接多个节点，表达完整的药物相互作用。

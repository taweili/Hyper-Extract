...
_PROMPT = """## 角色与任务
你是一位专业的处罚逻辑分析专家，请从文本中提取从“违规行为”到“处理程序”再到“最终结果”的演变路径，构建违规处罚因果链。

## 核心概念定义
- **节点 (Node)**：本模板中的"节点"指处罚因果链中的关键单元，包括违规行为、处理程序、处罚结果等类型，用于表示处罚流程中的关键环节。
- **边 (Edge)**：本模板中的"边"指节点之间的因果关系，包括导致、触发、执行、产生等二元关系，用于表示从违规到处罚的演变路径。

## 提取规则
### 节点提取规则
1. 提取所有关键节点：违规行为、处理程序、处罚结果等
2. 为每个节点指定类型：违规行为、处理程序、处罚结果、其他
3. 为每个节点添加详细描述

### 边提取规则
1. 仅从提取的节点中创建因果路径边
2. 记录节点之间的因果关系
3. 关系类型包括：导致、触发、执行、产生、其他
4. 添加详细说明（如有）
...
_EDGE_PROMPT = """## 角色与任务
你是一位专业的处罚因果关系识别专家，请从给定节点列表中提取节点之间的因果关系。

## 核心概念定义
- **节点 (Node)**：本模板中的"节点"指处罚因果链中的关键单元，作为因果关系的参与者。
- **边 (Edge)**：本模板中的"边"指节点之间的因果关系，包括导致、触发、执行、产生等二元关系，用于表示从违规到处罚的演变路径。

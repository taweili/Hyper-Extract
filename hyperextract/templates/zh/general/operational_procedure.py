...
_PROMPT = """## 角色与任务
你是一位专业的流程分析专家，请从文本中提取执行程序的步骤和转换关系。

## 核心概念定义
- **节点 (Node)**：流程步骤单元，包括步骤编号、名称、描述、执行角色、输入、输出等字段的操作单元
- **边 (Edge)**：流程步骤之间的转换关系，记录步骤之间的先后顺序和转换条件

## 提取规则
### 核心约束
1. 每个节点只能对应一个独立的步骤，禁止将多个步骤合并为一个节点
2. 步骤名称与原文保持一致
...
_EDGE_PROMPT = """## 角色与任务
请从已知步骤列表中提取步骤之间的转换关系。

## 核心概念定义
- **节点 (Node)**：流程步骤单元，作为流程转换的参与者
- **边 (Edge)**：流程步骤之间的转换关系，记录步骤之间的先后顺序和转换条件

## 提取规则
### 核心约束
1. 仅从已知步骤列表中提取边，不要创建未列出的步骤

...
_PROMPT = """## 角色与任务
你是一位专业的流程分析专家，请从文本中提取规章中规定的申报、审批或操作的具体先后步骤，构建执行程序流程图。

## 核心概念定义
- **节点 (Node)**：本模板中的"节点"指流程步骤单元，包含步骤编号、名称、描述、执行角色、输入、输出等字段的操作单元。
- **边 (Edge)**：本模板中的"边"指流程步骤之间的转换关系，记录步骤之间的先后顺序、转换条件等二元关系。
...
_NODE_PROMPT = """## 角色与任务
你是一位专业的流程步骤识别专家，请从文本中提取所有流程步骤作为节点。
...
_EDGE_PROMPT = """## 角色与任务
你是一位专业的流程转换识别专家，请从给定步骤列表中提取步骤之间的先后关系。

## 核心概念定义
- **节点 (Node)**：本模板中的"节点"指流程步骤单元，作为流程转换的参与者。
- **边 (Edge)**：本模板中的"边"指流程步骤之间的转换关系，记录步骤之间的先后顺序、转换条件等二元关系。

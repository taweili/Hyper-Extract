...
class GameItemSchema(BaseModel):
    ...
    category: str = Field(..., description="物品分类：'武器'、'防具'、'消耗品'、'材料'等")
    item_data: Optional[str] = Field(
        None, description="物品数值属性、合成配方及具体的伤害/防御效果。"
    )
    description: Optional[str] = Field(
        None, description="获取方式（掉落/商店）、适用职业及背景设定故事。"
    )
...
3. **item_data**：物品提供的属性加成、各种效果及合成公式信息
4. **description**：物品的来源渠道、推荐适用的职业角色及背景传说

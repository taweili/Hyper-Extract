"""A simple temporal graph implementation using AutoGraph as the core engine."""
...
# ==============================================================================
# 1. Schema Definition - Consistent with original AutoTemporalGraph implementation
# ==============================================================================
...
class EdgeSchema(BaseModel):
    ...
    AutoTemporalGraphic_facts: Optional[list[str]] = Field(
        default_factory=list,
        description=(
            "A list of exact string copies of the AutoTemporalGraphic facts or sentences from the source text that provide evidence for this relationship. "
...
# ==============================================================================
# 2. Prompt Definition - Adapted from original AutoTemporalGraph implementation
# ==============================================================================


AutoTemporalGraph_FACTOID_EXTRACTION_PROMPT = """
Observation Date: {observation_date}

You are an expert factoid extraction engine. Your primary function is to read a news paragraph and its associated observation date, and then decompose the text into a comprehensive list of AutoTemporalGraphic, self-contained, and temporally-grounded facts.
...
### 1. AutoTemporalGraphic Factoids
...
AutoTemporalGraph_EDGE_EXTRACTION_PROMPT = """
Observation Date: {observation_date}
...
6. **Evidence (`AutoTemporalGraphic_facts`)**: The exact source sentences/facts supporting this edge.
...
#### 3. Evidence Attribution (AutoTemporalGraphic_facts)
- For every relationship extracted, identify the specific sentence(s) or AutoTemporalGraphic fact(s) in the source text that provide the evidence.
- **COPY EXACTLY**: Populate the `AutoTemporalGraphic_facts` list with the exact strings from the input text.
...
**Output**:
- (Michel, is_CFO_of, Acme Corp, t_start=["2019-01-01"], t_end=["2021-01-01"], AutoTemporalGraphic_facts=["Michel served as CFO at Acme Corp from 2019 to 2021."])
- (Michel, works_at, Beta Inc, t_start=["2021-01-01"], t_end=["2023-01-01"], AutoTemporalGraphic_facts=["He was hired by Beta Inc in 2021, but left that role in 2023."])

**Input**: "Sarah was a board member of GreenFuture until 2019."
**Output**:
- (Sarah, is_board_member_of, GreenFuture, t_start=[], t_end=["2019-01-01"], AutoTemporalGraphic_facts=["Sarah was a board member of GreenFuture until 2019."])

**Input**: "(Observation Date: 2024-06-15) John Doe is no longer the CEO of GreenIT since a few months ago."
**Output**:
- (John Doe, is_CEO_of, GreenIT, t_start=[], t_end=["2024-03-15"], AutoTemporalGraphic_facts=["John Doe is no longer the CEO of GreenIT since a few months ago."])
  *(Reasoning: "a few months ago" from June is approx. March)*

**Input**: "John Doe's marriage is happening on 26-02-2026."
**Output**:
- (John Doe, has_status, Married, t_start=["2026-02-26"], t_end=[], AutoTemporalGraphic_facts=["John Doe's marriage is happening on 26-02-2026."])
...
- Ensure `AutoTemporalGraphic_facts` contains exact copies of the source text strings.
"""
...
6. **AutoTemporalGraph_facts**: Merge all evidence strings into a single list. Remove exact duplicate strings.
"""

# ==============================================================================
# 3. AutoTemporalGraph Implementation - Inherits from AutoGraph
# ==============================================================================


class AutoTemporalGraph(AutoGraph[NodeSchema, EdgeSchema]):
    """
    AutoTemporalGraph: A specialized AutoGraph for extracting high-quality triple-based KGs.

    Features:
    - Two-stage extraction: Extracts AutoTemporalGraphic facts first, then derives edges and nodes from facts
    - Customized prompts from original AutoTemporalGraph implementation
    ...
    - Evidence attribution: AutoTemporalGraphic_facts field traces each extracted edge to source facts
    ...
    Example:
        ...
        >>> kg = AutoTemporalGraph(llm_client=llm, embedder=embedder)
    ...
    def __init__(
        ...
        Args:
            ...
            facts_per_chunk: Max number of AutoTemporalGraphic facts to group into a single extraction batch (default: 20)
        ...
        logger.info("🚀 Initializing AutoTemporalGraph")
        super().__init__(
            ...
            prompt_for_edge_extraction=AutoTemporalGraph_EDGE_EXTRACTION_PROMPT,
    ...
    def _extract_data(self, text: str) -> AutoGraphSchema:
        """Extract AutoTemporalGraphic facts first, then extract edges (Two-stage extraction).

        Process:
        1. Split text into chunks.
        2. Batch extract AutoTemporalGraphic facts from chunks.
        ...
        # ==================== Step 1: Extract AutoTemporalGraphic Facts ====================
        logger.info("🔍 [Phase 1] Extracting AutoTemporalGraphic Facts...")
        ...
        logger.info("🔍 [Phase 2] Extracting Edges from AutoTemporalGraphic Facts...")
    ...
    def match_nodes_and_update_edges(
        ...
        Returns:
            The updated AutoTemporalGraph instance with matched nodes and updated edges.
        """
        logger.info(
